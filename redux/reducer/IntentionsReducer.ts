...
const intentionsReducer = (state = initialState, action: TIntentionsActionsTypes) => {
  let act: TIntentionsActionsTypes
  switch (action.type) {
    case constants.INTENTIONS.INT_ADD:
      act = action as IIntentionsAdd
      return { ...state, intentions: [ ...state.intentions, act.intention ], count: state.count + 1 }
    case constants.INTENTIONS.INT_UPDATE:
      act = action as IIntentionsUpdate
      return { ...state, intentions: act.intentions, count: act.intentions.length }
